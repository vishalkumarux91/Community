...
import { FORUM_POSTS as POSTS, type ForumPost as Post } from "@/data/forum";
      ...
      <aside className="space-y-5">
        <Card className="space-y-3 p-5" hover={false}>
          ...
          <ul className="space-y-2 text-sm">
            {[
              ["Critique my design", "/forum?type=critique"],
              ["Portfolio reviews", "/forum?type=portfolio-review"],
              ["Job prep", "/forum?type=job-prep"],
              ["Tools & AI", "/forum?tag=tools"],
              ["Career & switching", "/forum?tag=career"],
            ].map(([label, href]) => (
              <li key={label}>
        ...
        <Card className="space-y-3 p-5" hover={false}>
          ...
          <div className="space-y-2 text-sm text-text-weak">
            <div>
              <p className="font-medium text-text-strong">Ship client-ready sites with Claude</p>
              <p className="text-xs">Wed 8:30 PM · Vineet Chopdekar</p>
            </div>
          </div>
          <Link href="/workshops" className="text-xs text-link">
            See the workshop →
          </Link>
        </Card>
...
function PostCard({ post }: { post: Post }) {
  ...
  return (
    <Card as="a" href={`/forum/post/${post.id}`} className="flex gap-4 p-5">
